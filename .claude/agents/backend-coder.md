...
- **Pin tool versions. Never `@latest`.** Every `go run github.com/.../@VERSION` in the Makefile uses an explicit semver tag — `goose@v3.27.1`, `swag@v1.16.2`, `golangci-lint@v2.11.4` (current pins). If the same tool exists as a runtime dep in `go.mod`, the Makefile pin must match. If the operator's other projects pin a particular version, default to *that* version. Operator flagged `@latest` on 2026-05-17.
