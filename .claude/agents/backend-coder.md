...
- **No pure-passthrough service methods that exist only so a handler can peek at the repository.** `func (s *Service) X(ctx) (T, error) { return s.repo.X(ctx) }` with no logic is glue. If a handler reads `Count` to decide whether to call `Register`, push the decision into `Register`: have it return `ErrRegistrationClosed` when the count is non-zero, and the handler just calls `Register` and maps the error. One call instead of two; the rule lives where the action is. The repository can keep its primitive query — it's the *service interface* that shouldn't expose the peek. Operator flagged `users.Service.CountUsers` on 2026-05-17.
- **Pin tool versions AND container image tags. Never `@latest`, never `image:NN` floating tags.** Same family of rule. Applies to: `go run .../@VERSION` Makefile invocations, `docker-compose.yaml` image fields, `testcontainers-go.Run(ctx, "image:tag", ...)` calls, `Dockerfile FROM image:tag` lines. Use exact patch versions (e.g. `postgres:16.14-alpine`, not `postgres:16-alpine`). Local-dev compose tag and CI service-container tag and testcontainers-go tag MUST match. Operator flagged the container case on 2026-05-17.
- **Pin tool versions. Never `@latest`.** (Same rule, restated for grep.) Every `go run github.com/.../@VERSION` in the Makefile uses an explicit semver tag — `goose@v3.27.1`, `swag@v1.16.2`, `golangci-lint@v2.11.4` (current pins). If the same tool exists as a runtime dep in `go.mod`, the Makefile pin must match. If the operator's other projects pin a particular version, default to *that* version. Operator flagged `@latest` on 2026-05-17.
- **No adapter types whose only job is renaming a method.** If `consumer.Iface.Foo(...)` and `producer.Concrete.Bar(...)` exist solely to be glued by a 3-line `adapter struct{ s *producer.Concrete }; func (a adapter) Foo(...) { return a.s.Bar(...) }` in the wiring layer, fix the names — align both to one domain verb and the producer satisfies the interface natively. Operator flagged `seriesCreatorAdapter` on 2026-05-17.
